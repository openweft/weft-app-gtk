...
```
weft-app-gtk (tray)                         weft-app-gtk --dashboard
───────────────────                         ────────────────────────
...
```sh
weft-app-gtk --config app.json --wg-config wireguard.json
```
...
```
   ┌─────────────────┐    ┌────────────────────────┐  ┌──────────────────┐
   │ ~/.ssh/id_ed25519│   │ freedesktop Secret      │  │ weft-app-gtk     │
...
weft-app-gtk --store-ssh-passphrase ~/.ssh/id_ed25519
...
`weft-app-gtk --print-pubkey` and pasting the output into the
server's allowlist.
