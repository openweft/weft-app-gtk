...
func setupShell(ctx context.Context, configPath, wgConfigPath, authToken string) (*shell.Shell, string, error) {
	...
	opts := shell.Options{
		...
		// The operator pre-stages an entry via `weft-app-gtk
...
func onReady(ctx context.Context, sh *shell.Shell, controlURL, authToken string, authCfg AuthConfig) func() {
	return func() {
		...
		mActive := systray.AddMenuItem("Weft "+version, "Active cluster + datacenter")
		mActive.Disable()
		systray.AddSeparator()
		...
		case <-t.C:
			...
			if activeFull != lastActive {
				if activeFull == "" {
					...
					activeLabel.SetTitle("Weft " + version + " — no datacenter reachable")
				} else {
					systray.SetTooltip("Weft " + version + " — " + activeFull)
